...
test.describe('New Todo', () => {
	...
	test('should append new items to the bottom of the list', async ({
		...
		await expect(todoCount).toHaveText(
			'3 items left'
		);
...
test.describe('Mark all as completed', () => {
	...
	test('should allow me to mark all items as completed', async ({
		...
		// Complete all todos.
		await page
			.getByLabel('Mark all as complete')
			.check();
...
test.describe('Item', () => {
	test('should allow me to mark items as complete', async ({
		...
		const secondTodo = page
			.getByTestId('todo-item')
			.nth(1);
		await expect(secondTodo).not.toHaveClass(
			'completed'
		);
	...
	test('should allow me to un-mark items as complete', async ({
		...
		await expect(firstTodo).not.toHaveClass(
			'completed'
		);
		await expect(secondTodo).not.toHaveClass(
			'completed'
...
test.describe('Routing', () => {
	...
	test('should highlight the currently applied filter', async ({
		...
		await expect(completedLink).toHaveClass(
			'selected'
		);
	});
});
